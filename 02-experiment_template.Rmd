...
# lastmod: 2022-05-11T13:11:20.068Z
---
...
You can access the template for the experimental plan on Wattle or a copy can be found within this repository. [DOWNLOAD NOW]("pdfs/3. Statistical Methods in Biology - Chapter 3.pdf").
...
1) Response/Outcome measure for the experiment;
...
3) The experimental conditions (groups for comparisons);

```{r}
#input question here
```

4) The number of replicates/sample size for each condition;

```{r}
#input question here
```

5) The experimental control(s);

```{r}
#input question here
```

6) Are their any potentially confounding factors (“nuisance factors”)? 

```{r}
#input question here
```

7) Briefly explain how they will be tracked/or mitigated.
